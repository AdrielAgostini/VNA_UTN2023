...
% Last Modified by GUIDE v2.5 20-Nov-2023 20:08:36
...
if(~isempty(calibrationOutput))
    handles.S11_open = calibrationOutput.S11_open;
    handles.S11_short = calibrationOutput.S11_short;
    handles.S11_match = calibrationOutput.S11_match;
    handles.openOffsetP1 = calibrationOutput.openOffsetP1;
    handles.shortOffsetP1 = calibrationOutput.shortOffsetP1;
    handles.S22_open = calibrationOutput.S22_open;
    handles.S22_short = calibrationOutput.S22_short;
    handles.S22_match = calibrationOutput.S22_match;
    handles.openOffsetP2 = calibrationOutput.openOffsetP2;
    handles.shortOffsetP2 = calibrationOutput.shortOffsetP2;
    handles.S11_thru = calibrationOutput.S11_thru;
    handles.S22_thru = calibrationOutput.S22_thru;
    handles.S21_thru = calibrationOutput.S21_thru;
    handles.S12_thru = calibrationOutput.S12_thru;
    handles.calibration_parameter(:,1) = handles.S11_open;
    handles.calibration_parameter(:,2) = handles.S11_short;
    handles.calibration_parameter(:,3) = handles.S11_match;
    handles.calibration_parameter(:,4) = handles.S22_open;
    handles.calibration_parameter(:,5) = handles.S22_short;
    handles.calibration_parameter(:,6) = handles.S22_match;
    handles.calibration_parameter(:,7) = handles.S11_thru;
    handles.calibration_parameter(:,8) = handles.S21_thru;
    handles.calibration_parameter(:,9) = handles.S12_thru;
    handles.calibration_parameter(:,10) = handles.S22_thru;
    ...
    handles.gammaOpenListP1{i} = calibrationOutput.S11_open;
    handles.gammaShortListP1{i} = calibrationOutput.S11_short;
    handles.gammaMatchListP1{i} = calibrationOutput.S11_match;
    handles.openOffsetListP1{i} = calibrationOutput.openOffsetP1;
    handles.shortOffsetListP1{i} = calibrationOutput.shortOffsetP1;
    handles.gammaOpenListP2{i} = calibrationOutput.S22_open;
    handles.gammaShortListP2{i} = calibrationOutput.S22_short;
    handles.gammaMatchListP2{i} = calibrationOutput.S22_match;
    handles.openOffsetListP2{i} = calibrationOutput.openOffsetP2;
    handles.shortOffsetListP2{i} = calibrationOutput.shortOffsetP2;
    handles.S11_thruList{i} = calibrationOutput.S11_thru;
    handles.S22_thruList{i} = calibrationOutput.S22_thru;
    handles.S21_thruList{i} = calibrationOutput.S21_thru;
    handles.reverserTList{i} = calibrationOutput.S12_thru;
...
if (handles.readyVNAList{handles.currentMeasurmentIndex} == 1)
    ...
    gamma_Complex = zeros (length(handles.freqVector),4);
    Mag = zeros (length(handles.freqVector),4);
    Mag_v = zeros (length(handles.freqVector),4);
    Pha = zeros (length(handles.freqVector),4);
    Pha_v = zeros (length(handles.freqVector),4);

    if (handles.DebugMode ==0)
         [gamma_Complex, Mag, Mag_v,Pha,Pha_v] =  acquireData(handles,'FULL');   
    else
        for i = 1:4
            gamma_Complex(:,i) = i; 
        end
 ...
 %{  
    ...
    f_name = 'DUT';
    k_name = '(1)';
    folder = char([base_folder,date,f_name,k_name]);
    [~,~,r_value]=mkdir(folder);

    while r_value = 0

    end
%}
    [gamma_Complex_Corr,gamma_Corr] = gammaCorrect(gamma_Complex,handles.calibration_parameter);

    [file, path] = uiputfile('./temp/forDebug/dut_meas.mat','Exportar Medicion'); %Prompt para guardar archivo
    if (path>0) 
    handles.S11_C = gamma_Complex_Corr(:,1);
    handles.S21_C = gamma_Complex_Corr(:,2);
    handles.S12_C = gamma_Complex_Corr(:,3);
    handles.S22_C = gamma_Complex_Corr(:,4);
    handles.gamma_Corr = gamma_Corr;
    handles.mod11V = Mag_v(:,1);
    handles.mod21V = Mag_v(:,2);
    handles.mod12V = Mag_v(:,3);
    handles.mod22V = Mag_v(:,4);
    handles.phase11V = Pha_v(:,1);
    handles.phase21V = Pha_v(:,2);
    handles.phase12V = Pha_v(:,3);
    handles.phase22V = Pha_v(:,4);
    ...
    creaSxP([frecs, gamma_Corr],'Medicion_Corregida','.');
    creaSxP([frecs,  Mag_v(:,1), Pha_v(:,1), Mag_v(:,2), Pha_v(:,2), Mag_v(:,3), Pha_v(:,3), Mag_v(:,4), Pha_v(:,4)],'Medicion_sinCorregir','.');
    sound(handles.doneSound,handles.doneFs);
    
    
    % ----OutputFile Dut Values in Complex----- %
        %{
            -En el archivo estan los valores medidos y corregidos
            -Ambos tienen la fase resuelta
        %}
    dut_values_names = ["Freq" "S11_m" "S21_m" "S12_m" "S22_m" "S11_c" "S21_c" "S12_c" "S22_c"];
    dut_values = [frecs, gamma_Complex, gamma_Complex_Corr];

    outputfile = './temp/forDebug/dut_meas.csv';

    writematrix(dut_values_names, outputfile);
    writematrix(dut_values, outputfile,'WriteMode','append');
    ...
    % ----OutputFile Dut Values in Complex----- %
        %{
            ...
            -Ambos tienen la fase resuelta
        %}
    dut_values_voltage_names = ["Freq" "mag_S11" "mag_S21" "mag_S12" "mag_S22" "pha_S11" "pha_S21" "pha_S12" "pha_S22"];
    dut_values_voltage = [frecs, Mag_v, Pha_v];

    outputfile = './temp/forDebug/dut_meas_voltage.csv';
    
    writematrix(dut_values_voltage_names,outputfile );
    writematrix(dut_values_voltage, outputfile,'WriteMode','append');

    % ----Backup Workspace Dut Values ----- %
        %{
            gamma_Complex y gamma_Complex_Corr tienen la dase resuelta
            Mag_v y Pha_v no
        %}
    save([path file],'frecs','gamma_Complex','gamma_Complex_Corr','Mag_v','Pha_v');
    guidata(hObject,handles);
    end
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
%{ handles    structure with handles and user data (see GUIDATA)
frecs = reshape(handles.freqVector, length(handles.freqVector),1);
[file, path] = uiputfile('./Mediciones/.s2p','Exportar Medicion');
file_corr = [file,'corrected'];
file_nocorr = [file, 'notcorrected'];
creaSxP([frecs, handles.gammaC],file_corr,path);
creaSxP([frecs,handles.gammaM],file_nocorr,path)
...
%     r.S_Parameters = S11;
    r.Z0 = 50;
    [file, path] = uiputfile('output.s2p','Exportar Medicion'); %Prompt para guardar archivo
...
% handles    structure with handles and user data (see GUIDATA)
    if (handles.configuredFlag == 0)
        ...
        Ard = 0;
        % ConexiÃ³n Arduino
        Port  = 'COM10'; %Poner el COM a donde se conecta el Arduino NANO
        Board = 'Nano3';
        V_ref = 1.8;
...
% --- Executes on button press in cal_import.
function cal_import_Callback(hObject, eventdata, handles)
% hObject    handle to cal_import (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[file, path] = uigetfile();
cal_import = load([path,file]);
display("calibracion  cargada");
handles.calibration_parameter = zeros (length(cal_import.S11_open),10);
handles.freqVector = cal_import.freqOSM;
handles.calibration_parameter(:,1) = cal_import.S11_open;
handles.calibration_parameter(:,2) = cal_import.S11_short;
handles.calibration_parameter(:,3) = cal_import.S11_match;
handles.calibration_parameter(:,4) = cal_import.S22_open;
handles.calibration_parameter(:,5) = cal_import.S22_short;
handles.calibration_parameter(:,6) = cal_import.S22_match;
handles.calibration_parameter(:,7) = cal_import.S11_thru;
handles.calibration_parameter(:,8) = cal_import.S21_thru;
handles.calibration_parameter(:,9) = cal_import.S12_thru;
handles.calibration_parameter(:,10) = cal_import.S22_thru;
set(handles.Acquire,'Enable', 'on');
i = handles.currentMeasurmentIndex;
    handles.readyVNAList{i} = 1;
guidata(hObject,handles);
...
% --- Executes on button press in import_dut.
function import_dut_Callback(hObject, eventdata, handles)
% hObject    handle to import_dut (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[file, path] = uigetfile();
cal_import = load([path,file]);
handles.freqVector = cal_import.freqOSM;
handles.gammaM = db2mag(cal_import.M_sC) .* exp(1i .* cal_import.F_sC .* pi/180);
handles.gammaC = gammaCorrect(handles.gammaM,handles.calibration_parameter);
display("DUT cargado");
guidata(hObject,handles);


% --- Executes on button press in Export_cal.
function Export_cal_Callback(hObject, eventdata, handles)
% hObject    handle to Export_cal (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
frecs = reshape(handles.freqVector, length(handles.freqVector),1);
[file, path] = uiputfile('./.s1p','Exportar Medicion');
names = ["S11 open", "S11 short","S11 match",...
    "S22 open", "S22 short","S22 match",...
    "S11 thru","S21 thru","S12 thru","S22 thru"];

for i = 1:10
creaSxP([frecs, handles.calibration_parameter(:,i)],names(i),path);
end


% --- Executes on button press in ref_Plane.
function ref_Plane_Callback(hObject, eventdata, handles)
% hObject    handle to ref_Plane (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    offset = str2num(get(handles.refplane_offset,'String'));
    handles.gamma_refplane = reference_plane (handles.gammaC,handles.freqVector,offset);
    guidata(hObject,handles);


function refplane_offset_Callback(hObject, eventdata, handles)
% hObject    handle to refplane_offset (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of refplane_offset as text
%        str2double(get(hObject,'String')) returns contents of refplane_offset as a double


% --- Executes during object creation, after setting all properties.
function refplane_offset_CreateFcn(hObject, eventdata, handles)
% hObject    handle to refplane_offset (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in save_rplane.
function save_rplane_Callback(hObject, eventdata, handles)
% hObject    handle to save_rplane (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    handles.gammaC = handles.gamma_refplane;
    guidata(hObject,handles);