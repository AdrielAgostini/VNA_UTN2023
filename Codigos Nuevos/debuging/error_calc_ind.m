...
%}
 ...
 epsilon = 2.1;
 v = c;
 lambda = (v./freqOSM)./sqrt(epsilon);
%{
offsetOpen = 35/1000;
offsetShort = 35/1000;
    offsetO = 1.*exp(-1i*(4.*pi.*offsetOpen./lambda));
    offsetS = -1.*exp(-1i*(4.*pi.*offsetShort./lambda));
%}
    ...
    e30 = zeros(length(freqOSM),1); %No calibramos Crosstalk
    e10e32 = (S21_thru-e30).*(1-e11.*e22);

    %--Calibracion Puer , "to 2--%
    ...
    e03_r = zeros(length(freqOSM),1); %No calibramos Crosstalk
 ...
 %{
    Guardo los errores de la calibracion 
 %}
output_file = "error.mat";

%{
r= {'freqOSM' , 'e00' , 'e11' , 'e10e01' , 'deltae' , 'e22' , 'e30' , 'e10e32' ,...
     'e33_r' , 'e22_r' , 'e23e32_r' , 'deltae_r' , 'e11_r' , 'e03_r' , 'e23e01_r' ...
        } ; 
        %}
save(output_file,'freqOSM' , 'e00' , 'e11' , 'e10e01' , 'deltae' , 'e22' , 'e30' , 'e10e32' ,...
    'e33_r' , 'e22_r' , 'e23e32_r' , 'deltae_r' , 'e11_r' , 'e03_r' , 'e23e01_r' ...
    )
...
%{
path = "../Mediciones/01_11_23";

list = dir(fullfile("../Mediciones/01_11_23", '**', '*_converted.csv'));
...
%for k= 1:length(file)
   ...
   % aux_struct = csvread(aux_path,1);
    ...
    S21_M = db2mag(M_sC(:,2)).*exp(1i.*F_sC(:,2).*pi./180);
    S12_M = db2mag(M_sC(:,3)).*exp(1i.*F_sC(:,3).*pi./180);
    ...
    corrected_values (:,5)= angle(S11_C);
    corrected_values (:,6)= angle(S21_C);
    corrected_values (:,7)= angle(S12_C);
    corrected_values (:,8)= angle(S22_C);

    corrected_values (:,5:8) = corrected_values (:,5:8) * 180 / pi;
    
    output_file2 = "correciones.mat";
    output_file3 = "dut_corregido.mat";
%{
    r2 = [ 'freqVector' , 'N11' , 'N12' , 'N21' , 'N22' , 'D' ];
    r3 = ['freqVector', 'corrected_values'];
    %}
    %Output file 2 tiene los valores de N y D en complejo
    save(output_file2,'freqVector' , 'N11' , 'N12' , 'N21' , 'N22' , 'D')
    %Output file 3 tiene los valores de modulo y fase del dut corregidos
    save(output_file3,'freqVector', 'corrected_values')
    %{
