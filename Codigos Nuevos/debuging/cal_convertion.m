path = "../Mediciones/01_11_23/calibracion/";
...
transformed_f = converted_v;
for m = 3:2:21
   phasevector=-converted_v(1,m);
end
phasevector_corrected = correcion_fase ;

r= [freqVector,converted_v(:,2:21)];
r2= [freqVector,transformed_f(:,2:21)];
output_file=string(strcat(path,'measured_converted.csv'));
output_file2=string(strcat(path,'phase_pretransformed.csv'));
...
output_file=string(strcat(path,'/calibration_complex_value.mat'));
